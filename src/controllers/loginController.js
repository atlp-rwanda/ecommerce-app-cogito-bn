...
export const checkUser = async (req, res, next) => {
  ...
  if (!email || !password) {
    return res.status(400).json({ status: 400, message: req.t('Provide Email and Password Message') })
  }
  if (!user) {
...
if (!user) {
  ...
  if (password === user.password) {
    ...
    return res.status(200).json({ status: 200, user, token, message: req.t('Login Successfully message') });
